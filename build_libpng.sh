...
build_target() {
    ...
    BUILD_DIR="${SCRIPT_DIR}/build/libpng/${TARGET}"
    INSTALL_DIR="${SCRIPT_DIR}/install/libpng/${TARGET}"
    ...
    CMAKE_ARGS=(
        ...
        -DPNG_TESTS=OFF
        -DPNG_TOOLS=OFF
        -DPNG_LIBCONF_HEADER="${LIBPNG_DIR}/pnglibconf.h.prebuilt"
        -DZLIB_ROOT="${ZLIB_NG_INSTALL_DIR}"
        -DZLIB_USE_STATIC_LIBS=ON
        -DCMAKE_PREFIX_PATH="${ZLIB_NG_INSTALL_DIR}"
    )
    ...
    echo "libpng 빌드 완료 (${TARGET}): ${INSTALL_DIR}"
    echo ""
}
