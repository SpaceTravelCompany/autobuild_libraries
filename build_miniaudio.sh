...
build_target() {
    ...
    if [ "$ANDROID_ONLY" = true ]; then
        ...
        clang -c miniaudio_libopus.c -fPIC -O3 -I"${OGG_INCLUDE_DIR}" -I"${OPUS_INCLUDE_DIR}" -I"${OPUSFILE_INCLUDE_DIR}" --target=${TARGET} ${SSE4}
        ar r libminiaudio_libopus.a miniaudio_libopus.o
        clang -c miniaudio_libvorbis.c -fPIC -O3 -I"${OGG_INCLUDE_DIR}" -I"${VORBIS_INCLUDE_DIR}" --target=${TARGET} ${SSE4}
        ...
        CCFLAGS="-O3 -msse4.1 -fms-runtime-lib=static"
        ...
        clang-cl -c ${CCFLAGS} miniaudio_libvorbis.c -I"${OGG_INCLUDE_DIR}" -I"${VORBIS_INCLUDE_DIR}"
